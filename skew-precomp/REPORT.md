...
# 第 5 轮：skew 扩展到 1 ps（μ ≤ 0.266）

一阶结构在 ±1 ps 不够用（第 2 轮：仅 −18 dB）——偶次误差项
cos(ωμ)−1 ≈ −(ωμ)²/2 在带边缘达 −0.33，必须补二阶项。但第 4 轮的
零乘法器思路完整保留：**μ̂ 和 μ̂² 都是准静态标量，各自用 ≤2 项 SPT
移位实现**：

```
v1[n] = Σₖ aₖ·(x[n−k] − x[n+k])          k = 1..6   （反对称，一阶）
v2[n] = b₀·x[n] + Σₖ bₖ·(x[n−k] + x[n+k]) k = 1..2   （对称，二阶）
y[n]  = x[n] + s1·v1[n] + s2·v2[n]        s1 ≈ μ, s2 ≈ μ²，各 ≤2 项 SPT
```

`sim_1ps.py` 的结论（最坏值 = 密集 μ 网格 × 全部 β）：

| 配置 | 最坏 MSE |
|---|---|
| K1=5, K2=2，浮点标量 | −26.2 dB |
| **K1=6, K2=2，浮点标量** | **−27.8 dB** |
| 同上 + s1/s2 各 ≤2 项 SPT | −27.8 dB（无损！） |
| 同上 + d_k 截到 6 bit | **−27.2 dB** |
| 同上 + d_k 截到 5 bit | −25.9 dB（临界可用） |

要点：

1. **二阶子滤波器可以很短**：K2 = 2（5 抽头对称）就饱和，因为二阶
   修正本身幅度小（∝μ²），其误差被再衰减 ~11 dB。K2 = 3 无增益。
2. **SPT 标量近似零损失**：s1 ≤2 项、s2 ≤2 项（甚至 s2 = 1 项只损
   0.4 dB）。移位挡位窗口验证：s1 用 {≫2..≫7}（6 挡 + off = 7:1 mux），
   s2 用 {≫4..≫8}（5 挡 + off = 6:1 mux），−27.24 dB 不变。
3. **截位预算比 0.35 ps 版紧一位**：μ 大了 ~9 dB，修正通路量化噪声
   衰减变弱，d_k 需要 6 bit（0.35 ps 版是 5 bit）。
4. 硬件合计：约 30 个窄加法器 + 4 个静态移位 mux（7:1 ×2、6:1 ×2），
   **0 个乘法器**；固件写 {s1, s2} 两组 SPT 码（各 2×(符号+3bit 选择)）。

子滤波器系数（K1=6, K2=2，定点前）：
`a = [0.9534, −0.4369, 0.2665, −0.1767, 0.1202, −0.0795]`，
`b = [−1.537, 0.9030, −0.1719]`。

**±1 ps 方案排序（功耗视角）**：① DAC 时钟 PI（覆盖 ±1 ps 仍是宽松
指标，数字功耗为零）；② 上述零乘法器二阶 SPT-Farrow（−27.2 dB，
余量 2.2 dB）；③ 传统二阶 Farrow（2 个真乘法器，−26.3 dB，面积最省
但功耗高于②）。复现：`python3 sim_1ps.py`。
